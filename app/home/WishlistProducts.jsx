...
const WishlistProducts = () => {
  ...
  const sliderRef = useRef(null); // 🔹 moved here (valid hook)
  ...
  // 🔹 Fetch Wishlist – POST with user_id + company_id (localStorage)
  useEffect(() => {
    if (!isAuthReady) return;
    if (!isLoggedIn || !companyId) return; // only for logged in users (same as now)
    ...
    const fetchWishlist = async () => {
      try {
        ...
        console.error("⚠️ Error fetching wishlist:", err);
  ...
  if (!loading && (!wishlistItems || wishlistItems.length === 0)) {
    return null;
  }
  ...
  // 🔹 Single product card (reused for grid and slider)
  const renderProductCard = (product, extraWrapperClass = "") => {
    const now = new Date();
    ...
    return (
      ...
      >
        ...
        {/* Business Type Badges (if backend sends business_type in wishlist items) */}
        ...
        {/* Wishlist (still use WishlistButton so user can remove) */}
        {!isOutOfStock && (
      ...
      <div className="homepage-wrapper">
        ...
        {loading ? (
          ...
          wishlistItems.length > 0 && (
            <>
              {renderTitleSection("Wishlisted")}
              {renderProductCards(wishlistItems)}
            </>
          )
        )}
      </div>
